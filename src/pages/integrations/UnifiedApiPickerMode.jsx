...
export default function UnifiedApiPickerMode() {
  ...
  const handleConnect = async (integration) => {
    ...
    try {
      ...
      if (existingConn?.connectionid) {
        ...
        const patchBody = { integrationid: integration.integrationid, redirect: true };
        ...
        // POST new connection
        const postBody = { name: config.consumerName || 'Demo Consumer', integrationid: integration.integrationid, redirect: true };
  ...
  const handleDisconnect = async () => {
    ...
    setError(null);
    setConnecting(null);
    // Attempt to delete the connection on Chift; log in ApiLog and ignore errors in UI
          ...
          {techOpen && (
            <div className="border-top px-4 py-3" style={{ background: 'rgba(0,0,0,0.02)' }}>
              ...
              <div className="d-flex flex-column gap-2 small text-muted">
                ...
                <div><StepBadge n={7} />Return here — reload connection state</div>
              </div>
              {config.consumerId && (
